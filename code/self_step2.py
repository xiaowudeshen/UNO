...
'''
medium supervision, use main tasks and sub-tasks to select good labels for finetuining
...
def R1(args, training_data_path, output_file_R1):
    ...
    # pred_path1 = "t5_self/t5_flan_joint/google/flan-t5-smallt5_except_domain_hoteljoint_taskmask_slot_slotlang_question_lr_0.0001_epoch_5_seed_577/mask_slott5/R1/none/results/zeroshot_prediction.json"
    ################
    # breakpoint()
    preprocessed_data = data_preprocessing(training_data_path, pred_path1, output_file_R1,  purpose = 'prepare_slot')
    time1 = time.time()
    print(f"first step done, time taken {(time1-start_time)} result for mask slots are saved here: {output_file_R1}")
    next_step = 'R2'
    return pred_path1
...
def R2(args, training_data_path, pred_path1, output_file_R2, output_file_R1):
    ...
    time2 = time.time()
    time0 = time2
    print(f"second step done, time taken {(time2-start_time)} result for mask slots are saved here: {output_file_R2}")
    next_step = 'R3'
...
def R3(args):
    ...
    time3 = time.time()
    print(f"third step done, time taken {(time3-start_time)} result for mask slots are saved here: {pred_path3}")
    time0 = time3
    # args.fewshot = 0.0
    next_step = 'R1'
...
def self_training_step1(args, training_round):
    ...
    if not os.path.exists('data_self_2'):
        os.makedirs('data_self_2')
    ...
    print('Self-training done, total time taken is: ', time.time()-time0)
